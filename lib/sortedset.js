...
exports.zunionstore = function(mockInstance, destination, numKeys) {
  ...
  if (srcKeys.length === 0) {
    console.warn('Warning: No keys passed in to ZUNIONSTORE')
  }
  if(srcKeys.some( function(key){ return !key })) {
    console.warn('Warning: Undefined or null key(s) provided to ZUNIONSTORE:', srcKeys)
  }
...
var allObjsHaveKey = function(prop, objs){ return objs.every( function(o) { return o[prop] != null }) }

/* Sum of the given prop, as found in all the given objects */
var sumPropInObjs = function(prop, objs){
  return objs.reduce( function(sum, o){ return sum + Number(o[prop] || '0') }, 0)
}
...
exports.zinterstore = function(mockInstance, destination, numKeys) {
  ...
  var sources = srcKeys.map( function(srcKey){ 
    return mockInstance.storage[srcKey] != null ? mockInstance.storage[srcKey].value : null
