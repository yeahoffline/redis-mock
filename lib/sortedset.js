...
exports.zadd = function(mockInstance, key) {
  ...
  // declare opts
  var count = 0, nx = false, xx = false, ch = false, incr = false;
  var start = 2;
  ...
  for (var i = start; i < len; i += 2) {
    ...
    if (incr && existingScore) {
      score = parseFloat(existingScore, 10) + parseFloat(score, 10);
      score = score.toString();
    }
    // update score
    ...
    // if we have an incr (bhaves the same as zincrby)
    if (incr) {
      break;
    }
  }
...
exports.zincrby = function(mockInstance, key, increment, member, callback) {
  ...
  var score = parseFloat( s !== undefined ? s : '0', 10);
  increment = parseFloat(String(increment), 10);
  score += increment;
  mockInstance.storage[key].value[member] = score;
