...
const mockCallback = helpers.mockCallback;
...
exports.zadd = function(key) {
  const len = arguments.length;
  if (len <= 2) {
    return;
  }
  const callback = helpers.parseCallback(arguments);
  ...
  for (let i = start; i < len; i++) {
    const opt = arguments[i].toString().toLowerCase();
  ...
  for (let i = start; i < len; i += 2) {
    ...
    let score = arguments[i];

    // did we reach the end?
    if (len <= (i + 1)) {
        break;
    }
    const member = Item._stringify(arguments[i + 1]);
...
exports.zcount = function(key, min, max, callback) {
  const parse = (err, result) => {
    ...
    this._callCallback(callback, null, result.length);
  };
...
exports.zincrby = function(key, increment, member, callback) {
  ...
  score = String(score);
  this.storage[key].value[member] = score;
  this._callCallback(callback, null, score);
}
...
exports.zscore = function(key, member, callback) {
  ...
  this._callCallback(callback, null, (score === undefined ? null : score));
};
...
exports.zunionstore = function(destination, numKeys) {
  ...
  let callback = typeof c === 'function' && c || mockCallback;

  // Parse arguments
  let srcKeys = Array.from(arguments).slice(2, 2 + Number(numKeys))
    ...
    getRange(this, srcKey, 0, -1, 'withscores', (err, srcVals) => {
      ...
      if (!srcVals || srcVals.length === 0) {
        ...
        if (sourcesProcessed === srcKeys.length) {
          ...
          this._callCallback(callback, null, Object.keys(this.storage[destination].value).length)
          return;
        }
      }
      ...
      for(let i = 0; i < (srcVals.length -1); i = i+2) {
        //                                              score         member
        this.zadd(destination, srcVals[i+1], srcVals[i]);
        srcItemsProcessed++;
...
exports.zinterstore = function(destination, numKeys) {
  ...
  destValues.forEach((value) => {
    this.zadd(destination, dest[value], value)
  });

  this._callCallback(callback, null, destValues.length)
