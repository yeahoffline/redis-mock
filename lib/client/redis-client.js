...
class RedisClient extends events.EventEmitter {
  ...
  get _selectedDb() {
    return this._redisMock.select(this._selectedDbIndex);
  }
  ...
  _exec(parser, args, cb) {
    ...
    try {
      ...
      return this._callCallback(userCallback, err);
    }
  }
   ...
   *
   * TODO: move to a helper file. It doesn't require the "this" context
   */
  ...
  this._exec(parsers.set, userArgs, (args, cb) => {
    ...
    const postProcess = () => {
      if (expirationTime) {
        this._selectedDb.expire(args.default.key, expirationTime, (err, result) => {
          this._callCallback(cb, err, "OK");
        });
      } else {
        this._callCallback(cb, null, "OK");
      }
    };
    ...
    if ((keyExists && args.named.xx) || (!keyExists && args.named.nx) || (!args.named.xx && !args.named.nx)) {
      ...
      this._selectedDb.set(args.default.key, args.default.value, (err, result) => {
        if (err) {
          return this._callCallback(cb, err);
        }
        postProcess();
      });
    } else {
...
RedisClient.prototype.setex = RedisClient.prototype.SETEX = function (key, seconds, value, callback) {
  this._selectedDb.set(key, value, () => {
    this._selectedDb.expire(key, seconds, (err, result) => {
      this._callCallback(callback, err, "OK");
    });
  });
};
...
RedisClient.prototype.select = function (databaseIndex, callback) {
  ...
  if (!isNaN(databaseIndex) && (databaseIndex <= defaultMaxIndex)) {
    ...
    return this._callCallback(callback, null, "OK");
  } else {
    return this._callCallback(callback, new Error('ERR invalid DB index'), null);
  }
};
...
RedisClient.prototype.flushall = RedisClient.prototype.FLUSHALL = function (callback) {
  this._callCallback(callback, null, this._redisMock.flushall(callback));
};

RedisClient.prototype.auth = RedisClient.prototype.AUTH = function (password, callback) {
  this._callCallback(callback, null, this._redisMock.auth(password));
};
...
multi.Multi.prototype._callCallback = RedisClient.prototype._callCallback
