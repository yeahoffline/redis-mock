class Multi {
  ...
  _command(name, argList) {
    ...
    const command = args.concat((err, result) => {
      if(callBack) {
        this._callbacks.push(() => {
          ...
          if (this._callbacks.length !== 0) {
            this._client._callCallback(this._callbacks.shift());
          }
        });
      }
      ...
      if (this._commands.length === nextIndex) {
        ...
        this._client._callCallback(next)
      }
    });
  ...
  _runExec(callback) {
    ...
    if (this._discarded) {
      ...
      if (this._batch) {
        this._client._callCallback(callback, null, [err]);
      } else {
        this._client._callCallback(callback, err);
      }
    } else {
      if (!this._commands.length) {
        this._client._callCallback(callback, null, []);
      } else {
        this._commands[0]();
      }
    }
