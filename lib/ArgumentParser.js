...
const validateFlagDefinitions = (commandName, definitions) => {
  ...
  Object.values(definitions).forEach(flagDefinition => validateFlagDefinition(commandName, flagDefinition));
};

const getAllExclusivityKeys = definitions => {
  const result = new Set();
  Object.values(definitions.flags || {}).forEach(flagDefinition => {
    if (flagDefinition.exclusivityKey) {
      result.add(flagDefinition.exclusivityKey);
    }
  });

  Object.values(definitions.named || {}).forEach(namedDefinition => {
...
const validateExclusivityKeyDefinitions = (commandName, definitions) => {
  ...
  Object.keys(exclusivityKeyUsages).forEach(exclusivityKey => {
    ...
    if (usages.length < 2) {
      throw new Error('"exclusivityKey" should not be used if applies to only 1 property. Found: ' + key + ' on ' + usages[0].name);
    }
  });
};
...
module.exports.ArgumentParser = class {
  ...
  _validateMutualExclusivity(result) {
    ...
    Object.values(propertiesPerExclusivityKey).forEach(propertiesUsed => {
  ...
  parse(args) {
    ...
    while (index < args.length) {
      if (typeof args[index] !== 'string') {
        throw new Error('Expected ' + args[index] + ' to be a string');
      }
      const argName = args[index].toLowerCase();
      if (!this.definitions.interchangeableOrderDefinitions[argName]) {
        throw new Error('Failed to parse arguments for the command ' + this.commandName);
      }
