...
exports.hincrby = function (mockInstance, hash, key, increment, callback) {
  ...
  mockInstance.storage[hash].value[key] = parseInt(mockInstance.storage[hash].value[key], 10) || 0;

  mockInstance.storage[hash].value[key] += increment;

  mockInstance.storage[hash].value[key] += ""; //Because HGET returns Strings

  mockInstance._callCallback(callback, null, parseInt(mockInstance.storage[hash].value[key], 10)); //Because HINCRBY returns integers
};
...
exports.hlen = function (mockInstance, hash, callback) {
  ...
  for (var p in mockInstance.storage[hash].value) {
    if (mockInstance.storage[hash].value.hasOwnProperty(p)) { // eslint-disable-line no-prototype-builtins
      cnt++;
    }
  }
