...
const getAllClassMethods = (type) => {
  return Object.keys(type.prototype)
    .filter(name => name !== 'constructor' && typeof type.prototype[name] === 'function');
};

const getAllMethods = (objOrType) => {
  return typeof objOrType === 'object'
    ? getAllObjectMethods(objOrType)
    : getAllClassMethods(objOrType);
};

const extendMethodArray = (methodsArr) => {
  methodsArr.skip = (...skipArr) => {
    return extendMethodArray(methodsArr.filter(method => !skipArr.includes(method)));
  };
  methodsArr.add = (...addArr) => {
    return extendMethodArray(addArr.concat(methodsArr));
  };
  return methodsArr;
};
