 ...
 */
 var expireat = function (mockInstance, key, timestamp, callback) {
  ...
  if (obj) {
    ...
    mockInstance.storage[key].expires = new Date(timestamp);
...
exports.expire = function (mockInstance, key, seconds, callback) {
    ...
    var computedTimestamp = Date.now() + milli;
    return expireat(mockInstance, key, computedTimestamp, callback);
};

exports.pexpire = function (mockInstance, key, ms, callback) {
  var computedTimestamp = Date.now() + ms;
  return expireat(mockInstance, key, computedTimestamp, callback);
};
